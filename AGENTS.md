...
- **SSAO shader**: Alternative AO implementation using a procedural cosine-weighted hemisphere kernel with fixed per-pixel PCG3D jitter. Shares GTAO's depth, normal, and render-settings bindings and coordinate conventions; set 3 contains the 32-byte std140 `SSAOSettings` block (radius, bias, strength, power, sample count, distance falloff, and padding). Outputs grayscale visibility with white unoccluded; no extra noise texture, kernel buffer, or temporal history. Render-loop and blur integration are separate from the shader.
- **AO blur pass**: Spatially smooths the GTAO color mask with a compact 5x5 binomial (Gaussian-style) kernel folded into nine bilinear samples in one pass. Reads no depth, normals, or history; clamps sample coordinates to prevent edge wrapping. Writes a full-resolution, color-only framebuffer for the screen pass to multiply into scene color. AO-only filtering reduces grain but can soften contact shadows and bleed across geometry edges.
